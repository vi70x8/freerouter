...
function _getSessionKeyWithApiKey(
...
proxyRouter.post("/chat/completions", async (req: Request, res: Response) => {
	...
	const sessionKey =
		handoffMode !== "off" ? getSessionKey(messages, sessionIdHeader) : "";
	...
	outerLoop: for (let totalAttempt = 0; ; totalAttempt++) {
		...
		try {
			...
			if (err.code === "PINNED_MODEL_EXHAUSTED") {
				...
				publish({
					...
					id: requestId,
					cycle: oneRPMCycles,
					max: globalRetryMax > 0 ? globalRetryMax : null,
					reason: `Pinned model ${requestedModel} exhausted`,
					at: Date.now(),
				});
				console.log(
					`[Proxy] Pinned model ${requestedModel} exhausted, entering 1 RPM recovery (cycle ${oneRPMCycles}${globalRetryMax > 0 ? `/${globalRetryMax}` : "/∞"})`,
				);
				continue;
			}
			...
			console.log(
				`[Proxy] All models exhausted, entering 1 RPM recovery (cycle ${oneRPMCycles}${globalRetryMax > 0 ? `/${globalRetryMax}` : "/∞"})`,
			);
			continue;
		}
		...
		const _keySucceeded = false;
		for (let keyAttempt = 0; keyAttempt < PER_KEY_RETRIES; keyAttempt++) {
			try {
				if (stream) {
				...
				if (isRetryableError(err)) {
					...
					if (
						skipImmediately &&
						!(isPinned && route.modelDbId === preferredModel)
					) {
						skipModels.add(route.modelDbId);
						continue outerLoop;
					}
					...
					if (keyAttempt < PER_KEY_RETRIES - 1) {
						...
						console.log(
							`[Proxy] ${safeError.slice(0, 60)} from ${route.displayName}, retry ${keyAttempt + 1}/${PER_KEY_RETRIES} (same key)`,
						);
						continue;
					}
		...
		console.log(
			`[Proxy] Key ${route.keyId} exhausted after ${PER_KEY_RETRIES} failures from ${route.displayName}`,
		);
