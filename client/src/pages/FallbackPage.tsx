...
const STRATEGIES: { key: RoutingStrategy; label: string; blurb: string }[] = [
	...
	},
	{
		...
		label: "Custom",
		blurb:
			"Set your own balance of reliability, speed and intelligence with sliders. Same engine as the presets, just your weights.",
	},
];
		...
		<>
					...
					<div className="flex items-center gap-1.5">
						<div className="h-1.5 w-12 rounded-full bg-muted overflow-hidden">
							<div
								className="h-full rounded-full"
								style={{
									width: `${Math.round((row.speed ?? 0) * 100)}%`,
			...
			<td className="py-2 pr-3 align-middle text-right">
				<div className="flex items-center gap-1 justify-end">
					...
					<Switch
						checked={row.enabled}
						onCheckedChange={(c) => onToggle(row.modelDbId, c)}
					/>
				</div>
			</td>
		</>
...
function SortableRow({
	...
	row: Row;
	rank: number;
	onToggle: (id: number, e: boolean) => void;
	onEdit: (row: Row) => void;
}) {
	const {
		attributes,
...
export default function FallbackPage() {
	...
	const [pendingRetryLimit, setPendingRetryLimit] = useState<number | null>(
		null,
	);
	...
	function handleToggleAll(enabled: boolean) {
		setLocalEntries(
			allEntries.map((e) => (e.keyCount > 0 ? { ...e, enabled } : e)),
		);
	}
	...
	async function handleSaveAll() {
		...
		try {
			...
			if (pendingRetryLimit !== null) {
				await apiFetch("/api/fallback/retry-limit", {
					method: "PUT",
					body: JSON.stringify({ limit: pendingRetryLimit }),
				});
			...
			if (localEntries !== null) {
				...
				const rebased = [...enabled, ...disabled].map((e, i) => ({
					...e,
					priority: i + 1,
				}));
			...
			<div className="space-y-6">
					...
					<div className="flex items-baseline justify-between mb-3">
						...
						{routing?.weights && (
							<span className="text-xs text-muted-foreground tabular-nums">
								...
								{Math.round(routing.weights.intelligence * 100)}%
							</span>
						)}
					</div>
				...
				<section className="rounded-3xl border bg-card p-5">
					...
					<p className="text-xs text-muted-foreground mb-3">
						When all keys for all enabled models are rate-limited, the proxy
						enters 1 RPM recovery mode. Set how many recovery cycles before
						giving up (1–100), or 0 to keep retrying forever.
					</p>
					<div className="flex items-center gap-3">
						<input
							...
							onChange={(e) =>
								setPendingRetryLimit(parseInt(e.target.value, 10))
							}
									...
									>
										<div className="rounded-2xl border overflow-x-auto">
											<table className="w-full text-sm">
												{tableHead}
												<SortableContext
													items={ordered.map((e) => e.modelDbId)}
										...
										<table className="w-full text-sm">
											...
											<tbody>
												{ordered.map((row, i) => (
													<tr
														key={row.modelDbId}
														className={`border-b last:border-0 group ${row.enabled ? "" : "opacity-50"}`}
													>
														<RowContent
															row={row}
															rank={i + 1}
