...
interface RequestStartEvent extends LiveEventBase {
	type: "request.start";
	model?: string;
	stream: boolean;
}
interface RequestDoneEvent extends LiveEventBase {
	type: "request.done";
	model: string;
	provider: string;
	keyId: number;
...
interface RequestErrorEvent extends LiveEventBase {
	type: "request.error";
	error: string;
}
interface KeyExhaustedEvent extends LiveEventBase {
...
interface KeyRetryEvent extends LiveEventBase {
...
interface ModelSwitchEvent extends LiveEventBase {
	type: "routing.model_switch";
	from: string;
	to: string;
	reason: string;
}
interface RecoveryEvent extends LiveEventBase {
	...
	switch (evt.type) {
		...
		case "routing.key_retry":
			return {
				...
				ts,
				kind: "info",
				text: `↻ [${rId}] Retrying ${evt.provider}/${evt.model} key#${evt.keyId} (${evt.attempt}/${evt.max})`,
			};
		...
		case "routing.recovery":
			return {
				...
				ts,
				kind: "info",
				text: `⏳ [${rId}] Recovery cycle ${evt.cycle}${evt.max ? `/${evt.max}` : "/∞"}: ${evt.reason}`,
			};
	}
}
		...
		es.onmessage = (msg) => {
			try {
				...
				if (evt.type === "request.start") {
					...
					evt.type === "request.done" ||
					evt.type === "request.error"
				) {
